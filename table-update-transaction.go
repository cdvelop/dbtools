...
func (o operation) UpdateTablesTransaction(tx *sql.Tx, ctx context.Context, tables ...model.Object) bool {

	for _, table := range tables {
		//consulta entrega columna nombre
		q := fmt.Sprintf(o.SQLTableInfo(), table.Name)
		// tableInfo, ok := objectdb.QueryOne(q)
		tableInfo, ok := SelectOne(q, o.DB, ctx)
		...
		if len(tableInfo) == 0 { //si no existe crear tabla nueva
			...
			if list, ok := SelectOne("SELECT * FROM "+table.Name+";", o.DB, ctx); ok {

				if len(list) == 0 { //lista sin data borramos tabla y la creamos nuevamente para no chequearla
					if o.DeleteTableInTransaction(table, tx, ctx) {
					...
					if !o.ClonOneTableInTransaction(table, tx, ctx) { //clonamos la tabla con data a la nueva
